...
class LogParserService
{
    ...
    public function logUploadThroughBrowser($rows, $file_id)
    {
        ...
        try {
            ...
            foreach ($rows as $row => $data) {
                $data_keeper = $this->fillingTables($data);
                if (is_numeric($data_keeper[1][0][8]) === true) {
                    $collector[] = $this->loadformatone($data_keeper[0], $data_keeper[1], $file_id);
                } else
                    $collector[] = $this->loadformattwo($data_keeper[0], $data_keeper[1], $file_id);
            }
    ...
    public function logUploadThroughConsole($rows, $filename)
    {
        ...
        try {
            ...
            foreach ($rows as $row => $data) {
                $data_keeper = $this->fillingTables($data);
                if (is_numeric($data_keeper[1][0][8]) === true) {
                    $collector[] = $this->loadformatone($data_keeper[0], $data_keeper[1], $uploadedfile->filename_id);
                } else
                    $collector[] = $this->loadformattwo($data_keeper[0], $data_keeper[1], $uploadedfile->filename_id);
            }
            echo "Я заполнил массив и потратил на это: " . round(memory_get_usage($real_usage = true)/1000000, 3) . " МБ" . "\n";
            $this->saver($collector);
            $transaction->commit();
     ...
     * @return array
     */
    public function fillingTables($data)
    {
        /*
    ...
    public function loadformatone($row_data_edited, $row_data, $file_id)
    {
        /*
    ...
    public function loadformattwo($row_data_edited, $row_data, $file_id)
    {
        /*
         * элементам массива с данными присваиваются говорящие имена, которые соответсвуют
         * названиям столбцов в таблицах, но имеют приставку v_ для лучшей читаемости.
